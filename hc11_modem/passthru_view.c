...
static void start_stop_radio(App *app, PassthruModel *passthru_model) {
  ...
  if(passthru_model->enabled && passthru_model->vcp_connected) {
    ...
    /* Start the radio */
    start_radio(app->hc11_radio_comm_app, app->config.channel,
		app->config.address, app->config.txpower, app->config.mode);
...
static void log_serial_bytes(PassthruModel *passthru_model, bool to_radio,
...
static int32_t vcp_rx_tx_thread(void *ctx) {
  ...
  while(1) {
	...
	furi_stream_buffer_receive(passthru_model->vcp_rx_stream,
					...
					sizeof(passthru_model->tx_buf), 0);
      ...
      if(passthru_model->tx_buf_len) {
        ...
        if(passthru_model->enabled && passthru_model->vcp_connected) {
          ...
          radio_tx(app->hc11_radio_comm_app, passthru_model->tx_buf,
			passthru_model->tx_buf_len);

          /* Log the relayed bytes */
          log_serial_bytes(passthru_model, true, passthru_model->tx_buf,
				passthru_model->tx_buf_len);
	...
	furi_stream_buffer_receive(passthru_model->rx_stream,
