...
#define XINPUT_UI_TEXT(en, zh) (zh)
#else
#define XINPUT_UI_TEXT(en, zh) (en)
#endif
...
static void app_draw_callback(Canvas* canvas, void* ctx) {
	...
	if (!hid_connected) {
        ...
        canvas_draw_str(canvas, 60, 35, XINPUT_UI_TEXT("Connecting...", "连接中..."));
        canvas_draw_icon(canvas, 5, 20, &I_usb);
	} else {
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str(canvas, 74, 12, XINPUT_UI_TEXT("Controller", "手柄"));
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str(canvas, 64, 24, XINPUT_UI_TEXT("Hold", "长按"));
        canvas_draw_icon(canvas, 84, 16, &I_back);
        canvas_draw_str(canvas, 97, 24, XINPUT_UI_TEXT("to exit", "退出"));
