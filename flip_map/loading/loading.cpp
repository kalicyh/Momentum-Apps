...
void Loading::drawSpinner()
{
    ...
    canvas_set_font_custom(canvas, FONT_SIZE_SMALL);
    canvas_draw_str(canvas, 0, 60, FLIP_MAP_UI_TEXT("Time Elapsed:", "已用时间:"));
    char timeStr[24];
    ...
    if (seconds < 60)
    {
        ...
        {
            snprintf(
                timeStr, sizeof(timeStr), FLIP_MAP_UI_TEXT("%u second", "%u 秒"), seconds);
        }
        else
        {
            snprintf(
                timeStr, sizeof(timeStr), FLIP_MAP_UI_TEXT("%u seconds", "%u 秒"), seconds);
        }
    }
    else
    {
        snprintf(
            timeStr, sizeof(timeStr), FLIP_MAP_UI_TEXT("%u minutes", "%u 分钟"), seconds / 60);
    }
