...
void nfc_magic_scene_wipe_fail_on_enter(void* context) {
    ...
    widget_add_string_element(widget, 64, 0, AlignCenter, AlignTop, FontPrimary, NFC_MAGIC_UI_TEXT("Failed to Wipe", "擦除失败"));
    widget_add_string_multiline_element(
        widget, 0, 13, AlignLeft, AlignTop, FontSecondary, NFC_MAGIC_UI_TEXT("Something went\nwrong while wiping", "擦除过程中\n发生错误"));

    widget_add_button_element(
        widget, GuiButtonTypeLeft, NFC_MAGIC_UI_TEXT("Retry", "重试"), nfc_magic_scene_wipe_fail_widget_callback, instance);
