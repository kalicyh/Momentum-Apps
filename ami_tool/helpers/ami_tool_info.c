...
#define AMI_TOOL_INFO_UI_TEXT(en, zh) (zh)
#else
#define AMI_TOOL_INFO_UI_TEXT(en, zh) (en)
#endif
...
static const char* ami_tool_info_error_to_string(MfUltralightError error) {
    ...
    case MfUltralightErrorNone:
        return AMI_TOOL_INFO_UI_TEXT("No error", "无错误");
    case MfUltralightErrorNotPresent:
        return AMI_TOOL_INFO_UI_TEXT("Tag not present", "未检测到标签");
    case MfUltralightErrorProtocol:
        return AMI_TOOL_INFO_UI_TEXT("Protocol error", "协议错误");
    case MfUltralightErrorAuth:
        return AMI_TOOL_INFO_UI_TEXT("Authentication failed", "认证失败");
    case MfUltralightErrorTimeout:
        return AMI_TOOL_INFO_UI_TEXT("Timed out", "已超时");
    default:
        return AMI_TOOL_INFO_UI_TEXT("Unknown error", "未知错误");
    }
}
...
static bool ami_tool_info_target_tag_is_blank(const MfUltralightData* tag_data, const char** reason) {
    ...
    if(reason) {
        *reason = AMI_TOOL_INFO_UI_TEXT("Detected tag is not blank.", "检测到的标签不是空白标签。");
    }
    if(!tag_data || tag_data->type != MfUltralightTypeNTAG215 || tag_data->pages_total < 135) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag does not have enough pages.", "检测到的标签页数不足。");
        }
        return false;
    }
    ...
    if(tag_data->page[2].data[1] != 0x48) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag has unexpected internal bytes.", "检测到的标签内部字节异常。");
        }
        return false;
    }
    ...
    if((tag_data->page[2].data[2] != 0x00) || (tag_data->page[2].data[3] != 0x00)) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag has static lock bits set.", "检测到的标签已设置静态锁定位。");
        }
        return false;
    }
    ...
    if(memcmp(tag_data->page[3].data, capability_defaults, sizeof(capability_defaults)) != 0) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag has unexpected capability container bytes.", "检测到的标签容量容器字节异常。");
        }
        return false;
    }
    ...
    if(memcmp(tag_data->page[4].data, otp_defaults, sizeof(otp_defaults)) != 0) {
        if(reason) {
            *reason =
                AMI_TOOL_INFO_UI_TEXT("Detected tag OTP page is not blank.", "检测到的标签 OTP 页非空。");
        }
        return false;
    }
    ...
    for(uint16_t page = 5; page <= 129; page++) {
        if(memcmp(tag_data->page[page].data, pack_defaults, MF_ULTRALIGHT_PAGE_SIZE) != 0) {
            if(reason) {
                *reason = AMI_TOOL_INFO_UI_TEXT(
                    "Detected tag already contains user data.", "检测到的标签已包含用户数据。");
            }
            return false;
        }
    }
    ...
    if(memcmp(tag_data->page[130].data, dynamic_lock_defaults, sizeof(dynamic_lock_defaults)) != 0) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag has dynamic lock bits set.", "检测到的标签已设置动态锁定位。");
        }
        return false;
    }
    ...
    if(memcmp(tag_data->page[131].data, cfg0_defaults, sizeof(cfg0_defaults)) != 0) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag has non-default RF config.", "检测到的标签 RF 配置不是默认值。");
        }
        return false;
    }
    ...
    if(memcmp(tag_data->page[132].data, cfg1_defaults, sizeof(cfg1_defaults)) != 0) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag has non-default access config.", "检测到的标签访问配置不是默认值。");
        }
        return false;
    }
    ...
    if(memcmp(tag_data->page[133].data, pwd_defaults, sizeof(pwd_defaults)) != 0 &&
       memcmp(tag_data->page[133].data, unreadable_defaults, sizeof(unreadable_defaults)) != 0) {
        if(reason) {
            *reason =
                AMI_TOOL_INFO_UI_TEXT("Detected tag password is not blank.", "检测到的标签密码区非空。");
        }
        return false;
    }
    ...
    if(memcmp(tag_data->page[134].data, pack_defaults, sizeof(pack_defaults)) != 0 &&
       memcmp(tag_data->page[134].data, unreadable_defaults, sizeof(unreadable_defaults)) != 0) {
        if(reason) {
            *reason = AMI_TOOL_INFO_UI_TEXT(
                "Detected tag PACK bytes are not blank.", "检测到的标签 PACK 字节非空。");
        }
        return false;
    }
...
static void ami_tool_info_show_usage_page(AmiToolApp* app) {
    ...
    if(!app->usage_entries || app->usage_page_count == 0) {
        widget_add_text_scroll_element(
            ...
            124,
            60,
            AMI_TOOL_INFO_UI_TEXT(
                "No usage data available for this Amiibo.", "没有此 Amiibo 的使用数据。"));
    ...
    furi_string_cat_printf(
        app->text_box_store,
        AMI_TOOL_INFO_UI_TEXT("\e#Usage Info %u/%u\n\n", "\e#使用信息 %u/%u\n\n"),
        (unsigned)(app->usage_page_index + 1),
        (unsigned)app->usage_page_count);
    furi_string_cat_printf(
        app->text_box_store, AMI_TOOL_INFO_UI_TEXT("Game: %s\n", "游戏: %s\n"), entry->game);
    if(entry->has_usage && entry->usage && entry->usage[0] != '\0') {
        furi_string_cat(app->text_box_store, AMI_TOOL_INFO_UI_TEXT("Usage: ", "用途: "));
        furi_string_cat(app->text_box_store, entry->usage);
        furi_string_push_back(app->text_box_store, '\n');
    }
    furi_string_cat_printf(
        app->text_box_store,
        AMI_TOOL_INFO_UI_TEXT("Writable: %s\n", "可写: %s\n"),
        entry->writable ? AMI_TOOL_INFO_UI_TEXT("Yes", "是") : AMI_TOOL_INFO_UI_TEXT("No", "否"));
    ...
    if(app->usage_page_index > 0) {
        widget_add_button_element(
            app->info_widget,
            GuiButtonTypeLeft,
            AMI_TOOL_INFO_UI_TEXT("Prev", "上页"),
            ami_tool_usage_button_callback,
            app);
    }
...
static void ami_tool_info_append_releases(FuriString* buffer, const char* releases) {
    ...
    furi_string_cat(buffer, AMI_TOOL_INFO_UI_TEXT("Released on:\n", "发售日期:\n"));
    ...
    while(*cursor) {
        ...
        if(sep) {
            *sep = '\0';
            ...
            if(*date == '\0') date = AMI_TOOL_INFO_UI_TEXT("unknown", "未知");
            furi_string_cat_printf(buffer, "  - %s: %s\n", region, date);
        } else {
            furi_string_cat_printf(buffer, "  - %s\n", entry);
        }
...
static void ami_tool_info_format_entry(
    ...
    const char* name = (field_count > 0 && fields[0]) ? fields[0] : AMI_TOOL_INFO_UI_TEXT("Unknown", "未知");
    const char* character =
        (field_count > 1 && fields[1]) ? fields[1] : AMI_TOOL_INFO_UI_TEXT("Unknown", "未知");
    const char* series =
        (field_count > 2 && fields[2]) ? fields[2] : AMI_TOOL_INFO_UI_TEXT("Unknown", "未知");
    const char* game_series =
        (field_count > 3 && fields[3]) ? fields[3] : AMI_TOOL_INFO_UI_TEXT("Unknown", "未知");
    const char* type =
        (field_count > 4 && fields[4]) ? fields[4] : AMI_TOOL_INFO_UI_TEXT("Unknown", "未知");
    ...
    ami_tool_info_append_field(
        app->text_box_store, AMI_TOOL_INFO_UI_TEXT("Character", "角色"), character);
    ami_tool_info_append_field(
        app->text_box_store, AMI_TOOL_INFO_UI_TEXT("Amiibo Series", "Amiibo 系列"), series);
    ami_tool_info_append_field(
        app->text_box_store, AMI_TOOL_INFO_UI_TEXT("Game Series", "游戏系列"), game_series);
    ami_tool_info_append_field(app->text_box_store, AMI_TOOL_INFO_UI_TEXT("Type", "类型"), type);
...
void ami_tool_info_show_page(AmiToolApp* app, const char* id_hex, bool from_read) {
    ...
    if(found) {
        ...
        if(from_read) {
            furi_string_cat(
                app->text_box_store,
                AMI_TOOL_INFO_UI_TEXT("Valid NTAG215 detected.\n", "检测到有效的 NTAG215。\n"));
            if(id_hex && id_hex[0] != '\0') {
...
bool ami_tool_info_show_usage(AmiToolApp* app) {
    ...
    if(!app->info_last_has_id || app->info_last_id[0] == '\0') {
        ami_tool_info_show_action_message(
            app,
            AMI_TOOL_INFO_UI_TEXT(
                "Usage data requires a valid Amiibo ID.\nRead or generate one first.",
                "使用数据需要有效的 Amiibo ID。\n请先读取或生成一个。"));
        return false;
    }
    ...
    if(!success) {
        if(asset_error) {
            ami_tool_info_show_action_message(
                app,
                AMI_TOOL_INFO_UI_TEXT(
                    "Unable to open amiibo_usage.dat.\nInstall or update the assets.",
                    "无法打开 amiibo_usage.dat。\n请安装或更新资源。"));
        } else if(!found) {
            ami_tool_info_show_action_message(
                app,
                AMI_TOOL_INFO_UI_TEXT(
                    "No usage info is available for this Amiibo.", "没有此 Amiibo 的使用信息。"));
        } else {
            ami_tool_info_show_action_message(
                app,
                AMI_TOOL_INFO_UI_TEXT(
                    "Usage data is invalid.\nInstall or update the assets.",
                    "使用数据无效。\n请安装或更新资源。"));
        }
    }
...
static int32_t ami_tool_info_write_worker(void* context) {
    ...
    if(app->write_cancel_requested) {
        ami_tool_info_write_send_event(
            ...
            AMI_TOOL_INFO_UI_TEXT("Write cancelled.", "写入已取消。"));
        return 0;
    }
    ...
    if(!target) {
        ami_tool_info_write_send_event(
            ...
            AMI_TOOL_INFO_UI_TEXT("Unable to allocate tag buffer.", "无法分配标签缓冲区。"));
        return 0;
    }
    ...
    while(true) {
        ...
        snprintf(
            message,
            sizeof(message),
            AMI_TOOL_INFO_UI_TEXT("Unable to read tag: %s", "无法读取标签: %s"),
    ...
    if(target->type != MfUltralightTypeNTAG215) {
        ami_tool_info_write_send_event(
            app,
            AmiToolEventInfoWriteFailed,
            AMI_TOOL_INFO_UI_TEXT("Detected tag is not an NTAG215.", "检测到的标签不是 NTAG215。"));
        goto cleanup;
    }
    ...
    if(!target_uid || target_uid_len < 7) {
        ami_tool_info_write_send_event(
            app,
            AmiToolEventInfoWriteFailed,
            AMI_TOOL_INFO_UI_TEXT("Unable to read tag UID.", "无法读取标签 UID。"));
        goto cleanup;
    }
    ...
    if(!ami_tool_info_target_tag_is_blank(target, &blank_reason)) {
        ami_tool_info_write_send_event(
            ...
            blank_reason ? blank_reason :
                           AMI_TOOL_INFO_UI_TEXT(
                               "Detected tag is not blank.", "检测到的标签不是空白标签。"));
        goto cleanup;
    }
    ...
    if(status != AmiToolWriteStatusOk) {
        ...
        if((status == AmiToolWriteStatusIoError) && (failed_page != UINT16_MAX)) {
            snprintf(
                message,
                sizeof(message),
                AMI_TOOL_INFO_UI_TEXT("Write failed at page %u: %s", "第 %u 页写入失败: %s"),
                failed_page,
            ...
            snprintf(
                message,
                sizeof(message),
                AMI_TOOL_INFO_UI_TEXT("Write failed: %s", "写入失败: %s"),
                ami_tool_info_error_to_string(io_error));
        } else {
...
bool ami_tool_info_save_to_storage(AmiToolApp* app) {
    ...
    if(success) {
        ...
        snprintf(
            message,
            sizeof(message),
            AMI_TOOL_INFO_UI_TEXT("Saved Amiibo as\n%s", "Amiibo 已保存为\n%s"),
            file_name);
...
void ami_tool_info_show_actions_menu(AmiToolApp* app) {
    ...
    submenu_set_header(app->submenu, AMI_TOOL_INFO_UI_TEXT("Amiibo Actions", "Amiibo 操作"));
    submenu_add_item(
        app->submenu,
        AMI_TOOL_INFO_UI_TEXT("Emulate", "模拟"),
        AmiToolInfoActionMenuIndexEmulate,
        ami_tool_info_actions_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        AMI_TOOL_INFO_UI_TEXT("Usage Info", "使用信息"),
        AmiToolInfoActionMenuIndexUsageInfo,
        ami_tool_info_actions_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        AMI_TOOL_INFO_UI_TEXT("Change UID", "更改 UID"),
        AmiToolInfoActionMenuIndexChangeUid,
        ami_tool_info_actions_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        AMI_TOOL_INFO_UI_TEXT("Write to Tag", "写入标签"),
        AmiToolInfoActionMenuIndexWriteTag,
        ami_tool_info_actions_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        AMI_TOOL_INFO_UI_TEXT("Save to Storage", "保存到存储"),
        AmiToolInfoActionMenuIndexSaveToStorage,
        ami_tool_info_actions_submenu_callback,
        app);
...
void ami_tool_info_show_action_message(AmiToolApp* app, const char* message) {
    ...
    if(message && message[0] != '\0') {
        ...
        furi_string_cat(
            app->text_box_store,
            AMI_TOOL_INFO_UI_TEXT(
                "This action is not available yet.", "此操作暂不可用。"));
    }
    furi_string_cat(
        app->text_box_store,
...
void ami_tool_info_handle_write_event(AmiToolApp* app, AmiToolCustomEvent event) {
    ...
    case AmiToolEventInfoWriteStarted:
        ...
        furi_string_set(
            app->text_box_store,
            AMI_TOOL_INFO_UI_TEXT("Writing Amiibo...\nDo not remove the tag.", "正在写入 Amiibo...\n请勿移开标签。"));
        text_box_reset(app->text_box);
        text_box_set_text(app->text_box, furi_string_get_cstr(app->text_box_store));
        view_dispatcher_switch_to_view(app->view_dispatcher, AmiToolViewTextBox);
        break;
...
bool ami_tool_info_request_write_cancel(AmiToolApp* app) {
    ...
    furi_string_set(
        app->text_box_store,
        AMI_TOOL_INFO_UI_TEXT("Cancelling write...\nPlease wait.", "正在取消写入...\n请稍候。"));
    text_box_reset(app->text_box);
    text_box_set_text(app->text_box, furi_string_get_cstr(app->text_box_store));
    view_dispatcher_switch_to_view(app->view_dispatcher, AmiToolViewTextBox);
    furi_hal_nfc_abort();
