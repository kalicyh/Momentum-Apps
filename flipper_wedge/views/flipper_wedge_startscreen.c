...
void flipper_wedge_startscreen_draw(Canvas* canvas, FlipperWedgeStartscreenModel* model) {
    ...
    canvas_draw_str_aligned(canvas, 64, 2, AlignCenter, AlignTop, FLIPPER_WEDGE_UI_TEXT("Flipper Wedge", "Flipper 楔入"));

    // HID connection status
    canvas_set_font(canvas, FontSecondary);
    char status_line[32];
    if(model->usb_connected && model->bt_connected) {
        snprintf(status_line, sizeof(status_line), FLIPPER_WEDGE_UI_TEXT("USB: OK  BT: OK", "USB: OK  蓝牙: OK"));
    } else if(model->usb_connected) {
        snprintf(status_line, sizeof(status_line), FLIPPER_WEDGE_UI_TEXT("USB: OK  BT: --", "USB: OK  蓝牙: --"));
    } else if(model->bt_connected) {
        snprintf(status_line, sizeof(status_line), FLIPPER_WEDGE_UI_TEXT("USB: --  BT: OK", "USB: --  蓝牙: OK"));
    } else {
        snprintf(status_line, sizeof(status_line), FLIPPER_WEDGE_UI_TEXT("No HID connection", "无 HID 连接"));
    }
    ...
    if(model->display_state == FlipperWedgeDisplayStateIdle) {
        ...
        canvas_set_font(canvas, FontSecondary);
        if(connected) {
            canvas_draw_str_aligned(canvas, 64, 46, AlignCenter, AlignTop, FLIPPER_WEDGE_UI_TEXT("Scanning...", "扫描中..."));
        } else {
            canvas_draw_str_aligned(canvas, 64, 46, AlignCenter, AlignTop, FLIPPER_WEDGE_UI_TEXT("Connect USB or BT", "连接 USB 或蓝牙"));
        }

        // Show Settings hint
        canvas_draw_str_aligned(canvas, 64, 56, AlignCenter, AlignTop, FLIPPER_WEDGE_UI_TEXT("[OK] Settings", "[OK] 设置"));
    } else if(model->display_state == FlipperWedgeDisplayStateScanning) {
        // Scanning state
        canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, FLIPPER_WEDGE_UI_TEXT("Scanning...", "扫描中..."));
        canvas_set_font(canvas, FontSecondary);
        ...
        canvas_draw_str_aligned(canvas, 64, 28, AlignCenter, AlignTop, FLIPPER_WEDGE_UI_TEXT("Waiting...", "等待中..."));
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 64, 44, AlignCenter, AlignTop, model->status_text);
        ...
        // Show "Sent"
        canvas_draw_str_aligned(canvas, 64, 36, AlignCenter, AlignCenter, FLIPPER_WEDGE_UI_TEXT("Sent", "已发送"));
    }
}
