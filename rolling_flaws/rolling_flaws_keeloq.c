...
static bool is_open(RollingFlawsModel* model, KeeLoqData* data) {
    ...
    if(!any_mf &&
       furi_string_cmp(data->mf, rolling_flaws_setting_protocol_mf_name_get(model)) != 0) {
        ...
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("BAD MF", "厂商错误"));
        return false;
    }
    ...
    if(data->fix != rolling_flaws_setting_fix_get(model)) {
        FURI_LOG_I(
            ...
            data->fix);
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("BAD FIX", "固定码错误"));
        return false;
    }

    if((rolling_flaws_setting_fix_get(model) & 0xFFFFFFF) == 0) {
        FURI_LOG_I(TAG, "Fix is test. Not checking data.");
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("TEST", "测试"));
    ...
    if(data->enc != FAILED_TO_PARSE) {
        ...
        if(match_bits != 0) {
            ...
            if(fix_sn != enc_sn) {
                ...
                furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("BAD SN", "SN 错误"));
                return false;
            } else {
    ...
    if(distance == 0 && rolling_flaws_setting_replay_get(model)) {
        ...
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("REPLAY", "重放"));
        model->future_count = 0xFFFFFFFF;
        model->count = data->cnt;
        return true;
    }
    ...
    if(rolling_flaws_setting_count_zero_get(model) && data->cnt == 0) {
        ...
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("COUNT0", "计数" "0"));
        model->future_count = 0xFFFFFFFF;
    ...
    if(distance <= rolling_flaws_setting_window_next_get(model)) {
        ...
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("NEXT", "下一个"));
        model->future_count = 0xFFFFFFFF;
        model->count = data->cnt;
        return true;
    }
    ...
    if(distance <= rolling_flaws_setting_window_future_get(model)) {
        ...
        if(model->future_count > 0xFFFF) {
            ...
            furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("FUTURE", "未来"));
        ...
        if(future_gap > 0 && future_gap <= rolling_flaws_setting_window_future_gap_get(model)) {
            ...
            furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("GAP", "间隔"));
            model->future_count = 0xFFFFFFFF;
            model->count = data->cnt;
            return true;
        }
        ...
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("BAD GAP", "间隔错误"));
        model->future_count = data->cnt;
        return false;
    }

    FURI_LOG_I(TAG, "Signal must be from the past (non-future).");
    furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("PAST", "过去"));
    return false;
}
...
void decode_keeloq(RollingFlawsModel* model, FuriString* buffer, bool sync) {
    ...
    if(!sync) {
        ...
        furi_string_set(model->status, ROLLING_FLAWS_UI_TEXT("SYNCED", "已同步"));
    }
