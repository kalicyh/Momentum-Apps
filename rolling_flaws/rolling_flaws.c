...
void rolling_flaws_receive_signal_draw_callback(Canvas* canvas, void* model) {
    ...
    canvas_set_font(canvas, FontPrimary);
    canvas_draw_str(canvas, 13, 8, ROLLING_FLAWS_UI_TEXT("Rolling code receiver", "滚动码接收器"));
    canvas_set_font(canvas, FontSecondary);
    ...
    if(my_model->future_count > 0xFFFF) {
        canvas_draw_str(canvas, 2, 44, ROLLING_FLAWS_UI_TEXT("Future:   none", "未来:   无"));
    } else {
    ...
    if(my_model->opened) {
        ...
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str(canvas, 85, 33, ROLLING_FLAWS_UI_TEXT("CLOSED", "已关闭"));
    }
...
RollingFlaws* rolling_flaws_alloc() {
    ...
    submenu_add_item(
        app->submenu,
        ROLLING_FLAWS_UI_TEXT("Config", "配置"),
        RollingFlawsSubmenuIndexConfigure,
        rolling_flaws_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        ROLLING_FLAWS_UI_TEXT("Reset count to 0", "重置计数为 0"),
        RollingFlawsSubmenuIndexResetCountToZero,
        rolling_flaws_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        ROLLING_FLAWS_UI_TEXT("Transmit Signal", "发射信号"),
        RollingFlawsSubmenuIndexTransmit,
        rolling_flaws_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        ROLLING_FLAWS_UI_TEXT("Receive Signals", "接收信号"),
        RollingFlawsSubmenuIndexReceive,
        rolling_flaws_submenu_callback,
        app);
    submenu_add_item(
        app->submenu,
        ROLLING_FLAWS_UI_TEXT("Sync Remote", "同步遥控器"),
        RollingFlawsSubmenuIndexSyncRemote,
        rolling_flaws_submenu_callback,
        app);
    submenu_add_item(
        app->submenu, ROLLING_FLAWS_UI_TEXT("About", "关于"), RollingFlawsSubmenuIndexAbout, rolling_flaws_submenu_callback, app);
