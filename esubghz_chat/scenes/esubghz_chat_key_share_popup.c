...
void scene_on_enter_key_share_popup(void* context) {
    ...
    popup_set_header(state->nfc_popup, ESUBGHZ_CHAT_UI_TEXT("Sharing...", "分享中..."), 67, 13, AlignLeft, AlignTop);
    popup_set_icon(state->nfc_popup, 0, 3, &I_NFC_dolphin_emulation_51x64);
    popup_set_text(state->nfc_popup, ESUBGHZ_CHAT_UI_TEXT("Sharing\nKey via\nNFC", "通过 NFC\n分享密钥"), 90, 28, AlignCenter, AlignTop);
