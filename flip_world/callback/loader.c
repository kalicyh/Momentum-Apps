...
static void loader_error_draw(Canvas *canvas, DataLoaderModel *model)
{
    ...
    if (model->fhttp->last_response != NULL)
    {
        ...
        {
            canvas_clear(canvas);
            canvas_draw_str(canvas, 0, 10, FLIP_WORLD_UI_TEXT("[ERROR] Not connected to Wifi.", "[错误] 未连接 WiFi。"));
            canvas_draw_str(canvas, 0, 50, FLIP_WORLD_UI_TEXT("Update your WiFi settings.", "请更新 WiFi 设置。"));
            canvas_draw_str(canvas, 0, 60, FLIP_WORLD_UI_TEXT("Press BACK to return.", "按返回键返回。"));
        }
        ...
        {
            canvas_clear(canvas);
            canvas_draw_str(canvas, 0, 10, FLIP_WORLD_UI_TEXT("[ERROR] WiFi error.", "[错误] WiFi 错误。"));
            canvas_draw_str(canvas, 0, 50, FLIP_WORLD_UI_TEXT("Update your WiFi settings.", "请更新 WiFi 设置。"));
            canvas_draw_str(canvas, 0, 60, FLIP_WORLD_UI_TEXT("Press BACK to return.", "按返回键返回。"));
        }
        ...
        {
            canvas_clear(canvas);
            canvas_draw_str(canvas, 0, 10, FLIP_WORLD_UI_TEXT("[STATUS]Connecting to AP...", "[状态] 正在连接 AP..."));
        }
        else
        {
            canvas_clear(canvas);
            FURI_LOG_E(TAG, "Received an error: %s", model->fhttp->last_response);
            canvas_draw_str(canvas, 0, 10, FLIP_WORLD_UI_TEXT("[ERROR] Unusual error...", "[错误] 异常错误..."));
            canvas_draw_str(canvas, 0, 60, FLIP_WORLD_UI_TEXT("Press BACK and retry.", "按返回键重试。"));
        }
    }
    else
    {
        canvas_clear(canvas);
        canvas_draw_str(canvas, 0, 10, FLIP_WORLD_UI_TEXT("[ERROR] Unknown error.", "[错误] 未知错误。"));
        canvas_draw_str(canvas, 0, 50, FLIP_WORLD_UI_TEXT("Update your WiFi settings.", "请更新 WiFi 设置。"));
        canvas_draw_str(canvas, 0, 60, FLIP_WORLD_UI_TEXT("Press BACK to return.", "按返回键返回。"));
    }
}
...
void loader_draw_callback(Canvas *canvas, void *model)
{
    ...
    {
        canvas_draw_str(canvas, 0, 7, FLIP_WORLD_UI_TEXT("Wifi Dev Board disconnected.", "WiFi 开发板已断开。"));
        canvas_draw_str(canvas, 0, 17, FLIP_WORLD_UI_TEXT("Please connect to the board.", "请连接开发板。"));
        canvas_draw_str(canvas, 0, 32, FLIP_WORLD_UI_TEXT("If your board is connected,", "如果开发板已连接，"));
        canvas_draw_str(canvas, 0, 42, FLIP_WORLD_UI_TEXT("make sure you have flashed", "请确认已刷入"));
        canvas_draw_str(canvas, 0, 52, FLIP_WORLD_UI_TEXT("your WiFi Devboard with the", "最新版 FlipperHTTP"));
        canvas_draw_str(canvas, 0, 62, FLIP_WORLD_UI_TEXT("latest FlipperHTTP flash.", "固件到开发板。"));
        return;
    }
    ...
    canvas_draw_str(canvas, 0, 17, FLIP_WORLD_UI_TEXT("Loading...", "加载中..."));
    ...
    {
        canvas_draw_str(canvas, 0, 27, FLIP_WORLD_UI_TEXT("Fetching...", "获取中..."));
        return;
    }

    if (http_state == RECEIVING || data_state == DataStateRequested)
    {
        canvas_draw_str(canvas, 0, 27, FLIP_WORLD_UI_TEXT("Receiving...", "接收中..."));
        return;
    }

    if (http_state == IDLE && data_state == DataStateReceived)
    {
        canvas_draw_str(canvas, 0, 27, FLIP_WORLD_UI_TEXT("Processing...", "处理中..."));
        return;
    }

    if (http_state == IDLE && data_state == DataStateParsed)
    {
        canvas_draw_str(canvas, 0, 27, FLIP_WORLD_UI_TEXT("Processed...", "处理完成..."));
        return;
    }
}
