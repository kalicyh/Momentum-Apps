...
static bool renfe_regular_parse(FuriString* parsed_data, const MfClassicData* data) {
    ...
    do {
        ...
        furi_string_cat_printf(
            parsed_data, METROFLIP_UI_TEXT("Type: %s\n", "类型: %s\n"), card_type);
    ...
    if(app->data_loaded) {
        ...
        if(mfc_data) {
            ...
            if(!is_ida_vuelta) {
                ...
                widget_add_button_element(widget, GuiButtonTypeLeft, METROFLIP_UI_TEXT("History", "历史"), renfe_regular_widget_callback, app);
            }
            ...
            if(app->data_loaded) {
                ...
                if(mfc_data) {
                    ...
                    if(widget) {
                        ...
                        widget_add_text_scroll_element(widget, 0, 0, 128, 52, furi_string_get_cstr(parsed_data));
                        widget_add_button_element(widget, GuiButtonTypeRight, METROFLIP_UI_TEXT("Back", "返回"), renfe_regular_widget_callback, app);
                        view_dispatcher_switch_to_view(app->view_dispatcher, MetroflipViewWidget);
                    }
