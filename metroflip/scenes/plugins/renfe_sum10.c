...
static NfcCommand renfe_sum10_poller_callback(NfcGenericEvent event, void* context) {
    ...
    if(mfc_event->type == MfClassicPollerEventTypeCardDetected) {
        ...
        widget_add_button_element(widget, GuiButtonTypeLeft, METROFLIP_UI_TEXT("History", "历史"), renfe_sum10_widget_callback, app);
        widget_add_button_element(widget, GuiButtonTypeCenter, METROFLIP_UI_TEXT("Save", "保存"), metroflip_save_widget_callback, app);
        widget_add_button_element(widget, GuiButtonTypeRight, METROFLIP_UI_TEXT("Exit", "退出"), metroflip_exit_widget_callback, app);
...
static void renfe_sum10_on_enter(Metroflip* app) {
    ...
    if(app->data_loaded) {
        ...
        if(mfc_data) {
            ...
            widget_add_button_element(widget, GuiButtonTypeLeft, METROFLIP_UI_TEXT("History", "历史"), renfe_sum10_widget_callback, app);
            widget_add_button_element(widget, GuiButtonTypeCenter, METROFLIP_UI_TEXT("Delete", "删除"), metroflip_delete_widget_callback, app);
            widget_add_button_element(widget, GuiButtonTypeRight, METROFLIP_UI_TEXT("Exit", "退出"), metroflip_exit_widget_callback, app);
            ...
            if(app->data_loaded) {
                ...
                if(mfc_data) {
                    ...
                    if(widget) {
                        ...
                        widget_add_text_scroll_element(widget, 0, 0, 128, 52, furi_string_get_cstr(parsed_data));
                        widget_add_button_element(widget, GuiButtonTypeRight, METROFLIP_UI_TEXT("Back", "返回"), renfe_sum10_widget_callback, app);
                        view_dispatcher_switch_to_view(app->view_dispatcher, MetroflipViewWidget);
                    }
