...
void metroflip_scene_delete_on_enter(void* context) {
    ...
    if(success) {
        ...
        popup_set_header(popup, METROFLIP_UI_TEXT("Deleted", "已删除"), 80, 19, AlignLeft, AlignBottom);
        popup_enable_timeout(popup);
    } else {
        popup_set_icon(popup, 69, 15, &I_WarningDolphinFlip_45x42);
        popup_set_header(popup, METROFLIP_UI_TEXT("Error!", "错误!"), 13, 22, AlignLeft, AlignBottom);
