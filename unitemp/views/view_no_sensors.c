...
static void no_sensors_draw_callback(Canvas* canvas, void* model) {
    ...
    canvas_draw_str_aligned(
        canvas, 63, 10, AlignCenter, AlignCenter, UNITEMP_UI_TEXT("No sensors yet", "暂无传感器"));
    canvas_set_font(canvas, FontSecondary);
    const uint8_t x = 65, y = 32;
    canvas_draw_rframe(canvas, x - 4, y - 10, 54, 33, 3);
    canvas_draw_rframe(canvas, x - 4, y - 10, 54, 34, 3);
    canvas_draw_str(canvas, x, y, UNITEMP_UI_TEXT("Press OK", "按 OK"));
    canvas_draw_icon(canvas, x + 38, y - 8, &I_Ok_btn_9x9);
    canvas_draw_str(canvas, x, y + 9, UNITEMP_UI_TEXT("to add a", "添加"));
    canvas_draw_str(canvas, x, y + 18, UNITEMP_UI_TEXT("new sensor", "新传感器"));
}
